		...
		try {
			...
			if (fieldNames != null && fieldNames.size() == cellCountInOneRow) {
				for (int i = 0; i < fieldNames.size(); i++) {
					...
					// setter方法
					Method method = ReflectionUtils.getMethod(clazz, "set" + fieldName, type);
					// map中存放方法和参数类型
				...
				for (int i = 0; i < cellCountInOneRow; i++) {
					...
					Field field = ReflectionUtils.getField(clazz, cellValue);
					
					cellValue = leftStr + rightStr;
					// setter方法的参数
					Class<?> type = field.getType();
					// setter方法
					Method method = ReflectionUtils.getMethod(clazz, "set"+cellValue, type);
					// map中存放方法和参数类型
					Map<String, Object> map = new HashMap<>(2);
					map.put(methodKey, method);
					map.put(typeKey, type);
...
//			workbook = new XSSFWorkbook();
			...
			Field[] fields = ReflectionUtils.getFieldArray(clazz);
			...
			if (keepStatus) {
				...
				for (String propertyName : propertyNames) {
					Field field = null;
					try {
						field = ReflectionUtils.getField(clazz, propertyName);
					} catch (Exception e) {
						logger.error("get error->", e);
					}
					if (field != null) {
